    ...
    else 
    {
        ...
        if(c_flag == 1) //只需要输出每个文件成功匹配的行数
        {
            for(int i = 0; i < fileVec.size(); i++)
            {
                ...
                if((fileVec.size() == 1 && h_flag == 1) || (fileVec.size() > 1 && h_flag != 0)) //需要输出文件名的情况：单文件且带-H指令、多文件且不带-h指令
                {
            ...
            for(int i = 0; i < fileVec.size(); i++)
            {
                for(int j = 0; j < fileVec[i].contents.size(); j++) //查看第j-1行
                {
                    if(fileVec[i].output[j] == 1) //该行是匹配的,要输出
                    {
                        ...
                        strcat(gTerm.strout,colorLine(fileVec[i].contents[j],fileVec[i].matchPositions[j].first,fileVec[i].matchPositions[j].second).c_str());
                        strcat(gTerm.strout,"\n");
